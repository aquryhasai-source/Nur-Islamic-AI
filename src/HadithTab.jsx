...
// ─── Hadith Result Card ───────────────────────────────────────────────────────
function HadithCard({ h, i, expanded, setExpanded, bookmarks, onToggleBookmark, unlocked, navigateTo, gold, goldDim, goldBdr, cardBg, cardHdr, textClr }) {
  ...
  return (
    <div style={{ marginBottom:"10px", background:cardBg, border:`1px solid ${goldBdr}`, borderRadius:"12px", overflow:"hidden" }}>
...
export default function HadithTab({ initialTopic, bookmarks = [], onToggleBookmark, unlocked, navigateTo, lightMode, textSize = 1 }) {
  const [view,         setView]         = useState("books");   // "books" | "reader" | "search"
  ...
  const openBook = async (coll) => {
    ...
    try {
      ...
      setBookHadiths(hadiths.map(h => ({ ...h, collectionId: coll.id, collectionLabel: coll.label })));
  ...
  const cardProps = { bookmarks, onToggleBookmark, unlocked, navigateTo, gold, goldDim, goldBdr, cardBg, cardHdr, textClr };
      ...
      <div style={{ display:"flex", flexDirection:"column", flex:1, minHeight:0 }}>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"12px 16px 20px" }}>
          {loadingBook ? <Loader label={`Loading ${activeBook?.label}…`}/> : (
            <>
              {paged.map((h, i) => <HadithCard key={i} h={h} i={i} expanded={expanded} setExpanded={setExpanded} {...cardProps}/>)}
              {hasMore && (
