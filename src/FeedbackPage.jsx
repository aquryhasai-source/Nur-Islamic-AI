        ...
        {sent ? (
          ...
          <button onClick={submit} disabled={!message.trim() || loading}
            style={{ width:"100%", padding:"14px", borderRadius:"14px",
              background: message.trim() && !loading ? `linear-gradient(135deg,${gold},${lightMode?"#a07020":"#a8862e"})` : goldFaint,
              border: `1px solid ${message.trim() && !loading ? gold : goldBdr}`,
              color: message.trim() && !loading ? (lightMode?"#fff":"#0d1f14") : goldDim,
