...
const CompassSVG = ({ bearing = 0, qibla = 0, size = 280 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <circle cx={r} cy={r} r="8" fill="#0d1814" stroke="#c9a84c" strokeWidth="2"/>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", padding:"12px 16px", borderBottom:`1px solid ${goldBdr}`, background:headerBg, backdropFilter:"blur(14px)", flexShrink:0 }}>
        ...
        <div style={{ flex:1, textAlign:"center", color:gold, fontSize:`${16*textSize}px`, fontWeight:700, letterSpacing:"1px" }}>Qibla Direction</div>
        ...
        {qiblaAngle !== null ? (
          <>
            <div style={{ marginBottom:"16px", textAlign:"center" }}>
              <div style={{ color:goldDim, fontSize:`${11*textSize}px`, letterSpacing:"2px", textTransform:"uppercase", marginBottom:"4px" }}>Facing Mecca</div>
              <div style={{ color:gold, fontSize:`${28 * textSize}px`, fontWeight:800 }}>{Math.round(qiblaAngle)}°</div>
              <div style={{ color:textDim, fontSize:`${11*textSize}px` }}>from North</div>
            </div>
            ...
            <button onClick={() => setShowCityInput(v => !v)}
              style={{ marginTop:"14px", background:"none", border:`1px solid ${goldBdr}`, borderRadius:"10px", padding:"8px 20px", color:goldDim, fontSize:`${12*textSize}px`, cursor:"pointer", fontFamily:"Nunito,sans-serif" }}>
              Change city
            </button>
          </>
        ) : (
