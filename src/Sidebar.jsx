export default function Sidebar({ isOpen, onClose, unlocked, lightMode, setLightMode, onNavigate }) {
  ...
  const NAV_ITEMS = [
    { key:"profile",  icon:"👤", label:"Profile",           sub:"Usage · Name · Text size",  pro:false },
    { key:"bookmarks",icon:"🔖", label:"Bookmarks",         sub:"Saved ayahs & hadiths",     pro:true  },
    { key:"qibla",    icon:"🧭", label:"Qibla Direction",   sub:"Find the direction of prayer",pro:true },
    { key:"calendar", icon:"📅", label:"Islamic Calendar",  sub:"Hijri dates & events",      pro:true  },
    { key:"recent",   icon:"🕐", label:"Chat History",      sub:"Your recent questions",      pro:false },
    { key:"getpro",   icon:"⭐", label:"Get Pro",           sub:"Unlock all features",        pro:false },
    { key:"about",    icon:"ℹ️",  label:"About",             sub:"Version · Disclaimer",       pro:false },
  ];
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 18px", borderBottom:`1px solid ${goldBdr}`, background:headerBg, backdropFilter:"blur(10px)", flexShrink:0 }}>
          <div style={{ color:gold, fontSize:"18px", letterSpacing:"3px", fontWeight:800 }}>NŪR</div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"10px 12px 20px" }}>
          {NAV_ITEMS.map(({ key, icon, label, sub, pro }) => (
            <button key={key}
              ...
              <div style={{ flex:1 }}>
                <div style={{ color:textClr, fontSize:"14px", fontWeight:700, lineHeight:1.3 }}>{label}</div>
                <div style={{ color:textDim, fontSize:"11px", marginTop:"2px" }}>{sub}</div>
              </div>
              {pro && !unlocked && (
                <div style={{ background:`linear-gradient(135deg,${gold},${lightMode?"#a07020":"#a8862e"})`, color:lightMode?"#fff":"#0d1f14", fontSize:"9px", fontWeight:800, letterSpacing:"1px", padding:"3px 8px", borderRadius:"10px", flexShrink:0 }}>
                  PRO
                </div>
              )}
              {pro && unlocked && (
                <div style={{ color:"rgba(76,175,132,0.7)", fontSize:"11px", flexShrink:0 }}>✦</div>
              )}
              {!pro && <div style={{ color:goldDim, fontSize:"16px", flexShrink:0 }}>›</div>}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding:"14px 18px", borderTop:`1px solid ${goldBdr}`, textAlign:"center" }}>
          <div style={{ color:gold, fontSize:"16px", fontFamily:"Georgia,serif", marginBottom:"3px" }}>بِسْمِ اللّٰهِ</div>
          <div style={{ color:textDim, fontSize:"9px", letterSpacing:"1px" }}>Always consult a qualified scholar</div>
        </div>
      </div>
    </>
  );
