        ...
        <div style={{ flex:1, overflowY:"auto", padding:"10px 12px 20px" }}>
          {NAV_ITEMS.map(({ key, icon, label, sub }) => (
            <button key={key}
              ...
              {key === "getpro" && !unlocked
                ? <div style={{ background:`linear-gradient(135deg,${gold},${lightMode?"#a07020":"#a8862e"})`, color:lightMode?"#fff":"#0d1f14", fontSize:"9px", fontWeight:800, letterSpacing:"1px", padding:"3px 8px", borderRadius:"10px", flexShrink:0 }}>AD‑FREE</div>
                : key === "getpro" && unlocked
