...
export default function IslamicCalendarPage({ onBack, onOpenSidebar, lightMode, textSize = 1 }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", padding:"12px 16px", borderBottom:`1px solid ${goldBdr}`, background:headerBg, backdropFilter:"blur(14px)", flexShrink:0 }}>
        ...
        <div style={{ flex:1, textAlign:"center", color:gold, fontSize:`${16*textSize}px`, fontWeight:700, letterSpacing:"1px" }}>Islamic Calendar</div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"0 0 40px" }}>
        ...
        <div style={{ padding:"20px 20px 16px", textAlign:"center", borderBottom:`1px solid ${goldBdr}` }}>
          {loading ? (
            <div style={{ color:goldDim, fontSize:`${13*textSize}px` }}>Loading…</div>
          ) : hijriToday ? (
            <>
              <div style={{ color:goldDim, fontSize:`${10*textSize}px`, letterSpacing:"2px", textTransform:"uppercase", marginBottom:"8px" }}>Today</div>
            ...
            <div style={{ color:"#e07b54", fontSize:`${13*textSize}px` }}>Could not load date</div>
          )}
        </div>

        {upcomingEvents.length > 0 && (
          <div style={{ padding:"18px 20px", borderBottom:`1px solid ${goldBdr}` }}>
            <div style={{ color:goldDim, fontSize:`${10*textSize}px`, letterSpacing:"2px", textTransform:"uppercase", marginBottom:"12px" }}>Upcoming</div>
          ...
          <div style={{ padding:"20px" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"4px", marginBottom:"8px" }}>
              {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => (
                <div key={d} style={{ textAlign:"center", color:goldDim, fontSize:`${11*textSize}px`, fontWeight:700, padding:"6px 0" }}>{d}</div>
              ))}
            </div>
            {(() => {
              const today = new Date();
