...
export default function IslamicCalendarPage({ onBack, unlocked, navigateTo, lightMode, textSize = 1 }) {
  const [hijriToday, setHijriToday] = useState(null);
  const [calMonth,   setCalMonth]   = useState(null); // { year, month, days: [...] }
  ...
  // ── Blurred calendar UI (for generating the blur effect) ─────────────────
  const BlurredCalendar = () => (
    <div style={{ filter:"blur(6px)", opacity:0.45, pointerEvents:"none", userSelect:"none", padding:"20px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"4px" }}>
        {Array.from({ length: 30 }).map((_, i) => (
          <div key={i} style={{ aspectRatio:"1", borderRadius:"8px", background:i === 14 ? gold : goldFaint, border:`1px solid ${goldBdr}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
            <span style={{ color: i === 14 ? "#0d1f14" : textClr, fontSize:"12px", fontWeight: i === 14 ? 800 : 400 }}>{i + 1}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"0 0 40px" }}>
        ...
        {/* Today's Hijri date — always shown */}
        ...
        {/* Calendar grid — pro gated */}
        <div style={{ position:"relative" }}>
          <BlurredCalendar/>

          {!unlocked && (
            <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"14px", padding:"20px" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ color:gold, fontSize:`${16 * textSize}px`, fontWeight:700, marginBottom:"8px" }}>Full Calendar</div>
                <div style={{ color:textDim, fontSize:`${12 * textSize}px`, lineHeight:1.7 }}>Browse the complete Hijri calendar month by month</div>
              </div>
              <button onClick={() => navigateTo("getpro")}
                style={{ padding:"13px 32px", borderRadius:"50px", background:`linear-gradient(135deg,${gold},${lightMode?"#a07020":"#a8862e"})`, border:"none", color:lightMode?"#fff":"#0d1f14", fontSize:`${14 * textSize}px`, fontWeight:800, cursor:"pointer", fontFamily:"Nunito,sans-serif", boxShadow:`0 6px 20px ${lightMode?"rgba(122,88,16,0.28)":"rgba(201,168,76,0.22)"}` }}>
                🌙 Get Pro
              </button>
            </div>
          )}
