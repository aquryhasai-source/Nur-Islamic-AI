...
function UpgradePrompt({ onOpenUnlock }) {
  ...
  // ── FIX: default false (international) so button shows immediately ──────
  // Then update to true if user is detected in India. This prevents the
  // "button not showing" bug when ipapi.co is slow or blocked.
  ...
  useEffect(() => {
    fetch("https://ipapi.co/json/")
      ...
      .catch(() => {}); // silently fail — keeps default (international)
  }, []);
    ...
    <div style={{ padding:"8px 0 32px" }}>
      ...
      <div style={{ background:"linear-gradient(160deg,rgba(201,168,76,0.09),rgba(201,168,76,0.03))", border:"1px solid rgba(201,168,76,0.22)", borderRadius:"24px", overflow:"hidden" }}>
        <div style={{ padding:"28px 20px 20px", textAlign:"center", borderBottom:"1px solid rgba(201,168,76,0.1)" }}>
          <div style={{ fontSize:"20px", color:"#c9a84c", fontFamily:"Georgia,serif", marginBottom:"6px" }}>طَلَبُ الْعِلْمِ فَرِيضَةٌ</div>
          <div style={{ color:"rgba(201,168,76,0.45)", fontSize:"11px", letterSpacing:"1.5px", marginBottom:"16px" }}>"Seeking knowledge is an obligation" — Ibn Mājah</div>
          <div style={{ color:"rgba(255,255,255,0.6)", fontSize:"14px", lineHeight:1.8, marginBottom:"20px" }}>
            You've used all your free reflections today.<br/>Continue your journey without limits.
          </div>

          {/* Payment button — always visible (defaults to international, swaps to India if detected) */}
          ...
          <div style={{ color:"rgba(201,168,76,0.35)", fontSize:"11px", marginTop:"8px" }}>{primaryNote}</div>
          <div style={{ color:"rgba(201,168,76,0.3)", fontSize:"11px", marginTop:"3px" }}>One-time payment · Yours forever</div>
...
// ─── Main ChatTab ─────────────────────────────────────────────────────────────
export default function ChatTab({ remaining, setRemaining, unlocked, setUnlocked, profile, deviceId, lightMode }) {
      ...
      try {
        ...
        if (navigator.geolocation) {
          navigator.geolocation.getCurrentPosition(
            async ({ coords }) => {
              try { const t = await getPrayerTimes(coords.latitude, coords.longitude); setPrayerTimes(t); } catch {}
            },
            () => {
  ...
  const formatText = (text) => text.split("\n").map((line, i) => {
    const html = line.replace(
      ...
      m => `<span style="color:${gold};font-weight:700;font-style:italic">${m}</span>`
    );
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"20px 16px", maxWidth:"760px", width:"100%", margin:"0 auto", boxSizing:"border-box" }}>
        ...
        {isEmpty && !isLocked && (
          <div style={{ textAlign:"center", paddingTop:"32px" }}>
            <div style={{ fontSize:"26px", color:gold, marginBottom:"8px", textShadow:`0 0 30px ${lightMode?"rgba(122,88,16,0.25)":"rgba(201,168,76,0.35)"}` }}>
              بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ
            </div>
            <div style={{ color:goldDim, fontSize:"11px", letterSpacing:"2px", marginBottom:"32px" }}>
              In the Name of Allah, the Most Gracious, the Most Merciful
            </div>
            {profile.name && (
              <div style={{ color:lightMode?"rgba(26,15,0,0.4)":"rgba(255,255,255,0.4)", fontSize:"14px", marginBottom:"16px" }}>
                As-salamu alaykum, <strong style={{ color:gold }}>{profile.name}</strong> 🌙
              </div>
            )}
            <div style={{ width:"40px", height:"1px", background:`linear-gradient(90deg,transparent,${gold},transparent)`, margin:"0 auto 28px" }}/>
            <p style={{ color:lightMode?"rgba(26,15,0,0.38)":"rgba(255,255,255,0.38)", fontSize:"14px", marginBottom:"28px", lineHeight:1.8 }}>
              Ask anything about Islam — grounded in the Quran and authentic Hadith.
            </p>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"10px", justifyContent:"center" }}>
              {SUGGESTIONS.map((s, i) => (
                <button key={i} onClick={() => sendMessage(s)}
                  style={{ background:lightMode?"rgba(122,88,16,0.07)":"rgba(201,168,76,0.07)", border:`1px solid ${lightMode?"rgba(122,88,16,0.2)":"rgba(201,168,76,0.2)"}`, borderRadius:"24px", color:gold, padding:"9px 18px", fontSize:"13px", cursor:"pointer", fontFamily:"Nunito,sans-serif", transition:"all 0.2s" }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"18px" }}>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{ display:"flex", gap:"10px", alignItems:"flex-start", justifyContent:msg.role==="user"?"flex-end":"flex-start" }}>
                {msg.role === "assistant" && (
                  <div style={{ width:"32px", height:"32px", borderRadius:"50%", flexShrink:0, background:`linear-gradient(135deg,${lightMode?"#f0e4c0,#e6d4a0":"#1a3a22,#0d2018"})`, border:`1px solid ${lightMode?"rgba(122,88,16,0.3)":"rgba(201,168,76,0.3)"}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <StarIcon/>
                  </div>
                )}
                <div style={{ maxWidth:"82%",
                  background: msg.role==="user"
                    ? lightMode?"linear-gradient(135deg,rgba(122,88,16,0.14),rgba(122,88,16,0.07))":"linear-gradient(135deg,rgba(201,168,76,0.18),rgba(201,168,76,0.09))"
                    : lightMode?"rgba(255,255,255,0.65)":"rgba(255,255,255,0.04)",
                  border: msg.role==="user"
                    ? `1px solid ${lightMode?"rgba(122,88,16,0.25)":"rgba(201,168,76,0.28)"}`
                    : `1px solid ${lightMode?"rgba(122,88,16,0.12)":"rgba(255,255,255,0.08)"}`,
                  borderRadius: msg.role==="user" ? "18px 4px 18px 18px" : "4px 18px 18px 18px",
                  padding:"13px 16px", color:textClr, fontSize:"14px", lineHeight:1.7 }}>
                  {msg.role==="assistant" ? formatText(msg.content) : <p style={{ margin:0 }}>{msg.display || msg.content}</p>}
                </div>
                {msg.role==="user" && (
                  <div style={{ width:"32px", height:"32px", borderRadius:"50%", flexShrink:0, background:lightMode?"rgba(122,88,16,0.12)":"rgba(201,168,76,0.12)", border:`1px solid ${lightMode?"rgba(122,88,16,0.25)":"rgba(201,168,76,0.25)"}`, display:"flex", alignItems:"center", justifyContent:"center", color:gold }}>
                    <UserIcon/>
                  </div>
                )}
              </div>
            ...
            {loading && (
              <div style={{ display:"flex", gap:"10px", alignItems:"flex-start" }}>
                <div style={{ width:"32px", height:"32px", borderRadius:"50%", background:lightMode?"linear-gradient(135deg,#f0e4c0,#e6d4a0)":"linear-gradient(135deg,#1a3a22,#0d2018)", border:`1px solid ${lightMode?"rgba(122,88,16,0.3)":"rgba(201,168,76,0.3)"}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <StarIcon/>
                </div>
            ...
            <div style={{ display:"flex", gap:"10px", alignItems:"flex-end" }}>
              <div style={{ flex:1, background:inpFldBg, border:`1px solid ${inpFldBdr}`, borderRadius:"14px", padding:"11px 14px" }}>
                <textarea ref={textareaRef} value={input} onChange={e => { setInput(e.target.value); autoResize(); }} onKeyDown={handleKeyDown}
                  ...
                  style={{ width:"100%", background:"transparent", border:"none", outline:"none", color:textClr, fontSize:"14px", resize:"none", fontFamily:"Nunito,sans-serif", lineHeight:1.6 }}/>
              </div>
