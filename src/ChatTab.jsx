...
// ─── Citation parser ──────────────────────────────────────────────────────────
// Matches patterns like [Surah Al-Baqarah 2:255] or [Al-Kahf 18:10] or [2:255]
const QURAN_CITATION  = /\[(?:Surah\s+)?(?:[A-Za-z][A-Za-z'\-\s]*?\s+)?(\d{1,3}):(\d{1,3})(?:[^\]]{0,20})\]/g;
// Matches [Sahih Bukhari #1] [Tirmidhi 2516] etc.
const HADITH_CITATION = /\[((?:Sahih\s+)?(?:Bukhari|Muslim|Abu\s+Dawud|Tirmidhi|An-Nasai|Nasai|Ibn\s+Majah|Jami'[^\]]*?))[^\]]*?#?\s*(\d+)\]/gi;
...
export default function ChatTab({ remaining, setRemaining, unlocked, setUnlocked, profile, deviceId, lightMode, textSize = 1, navigateTo, onSwitchTab, onQuranSurah, onHadithSearch }) {
  ...
  const renderMessage = (text) => {
    ...
    while ((m = HADITH_CITATION.exec(combined)) !== null) {
      allMatches.push({ index:m.index, end:m.index+m[0].length, raw:m[0], type:"hadith", collection:resolveCollection(m[1]), number:m[2] });
    }
      ...
      if (match.type === "quran") {
        ...
        lines.push(
          <span key={`h-${match.index}`}
            onClick={() => { onSwitchTab("hadith"); if (match.collection) onHadithSearch(`${match.raw.replace(/[\[\]]/g,"")}`); }}
            style={{ color:gold, fontWeight:700, fontStyle:"italic", cursor:"pointer", textDecoration:"underline dotted", textUnderlineOffset:"3px" }}
            title="Open in Hadith tab">
            {match.raw}
          </span>
        );
      }
